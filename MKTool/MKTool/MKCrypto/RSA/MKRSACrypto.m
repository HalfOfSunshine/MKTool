...
#import "MKRSACrypto.h"

@implementation MKRSACrypto
 ...
 * -------RSA 字符串公钥加密-------
 ...
 @return 密文，加密后的字符串
 */
+(NSString *)encrypt:(NSString *)dataIn key:(NSString *)key iv:(NSData *)iv{
    if (dataIn.length == 0 || key.length == 0) {
        return nil;
    }
    NSData *data = [self encryptData:[dataIn dataUsingEncoding:NSUTF8StringEncoding] publicKey:key];
...
+(NSString *)decrypt:(NSString* )dataIn key:(NSString *)key iv:(NSString *)iv{
    ...
    NSData *data = [[NSData alloc] initWithBase64EncodedString:dataIn options:NSDataBase64DecodingIgnoreUnknownCharacters];
